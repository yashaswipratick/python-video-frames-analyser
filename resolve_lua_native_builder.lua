...
local function dialog(title, text)
    local shown = false
    if fu then
        shown = pcall(function()
            fu:AskUser(title, {{"Message", "Text", Text = text}})
        end)
    end
    if not shown and comp then
        pcall(function()
...
local function log(msg)
    local line = "RESOLVE LUA BUILDER: " .. tostring(msg)
    print(line)
end

local function fail(msg)
    local text = "RESOLVE LUA BUILDER ERROR\n\n" .. tostring(msg)
    print(text)
    dialog("Resolve Lua Builder", text)
end

local function succeed(msg)
    print(msg)
    dialog("Resolve Lua Builder", msg)
end

-- Visible startup marker. This proves Resolve actually executed this file.
dialog("Resolve Lua Builder", "STARTED\n\nThe updated resolve_lua_native_builder.lua is executing.\n\nNext: acquiring the Resolve API...")
log("STARTED")
...
function JSON.decode(text)
    ...
    local function parseString()
        if text:sub(pos, pos) ~= '"' then error("expected JSON string") end
        pos = pos + 1
        local out = {}
        while pos <= len do
            local c = text:sub(pos, pos)
    ...
    local function parseValue()
        ...
        if c == "{" then
            ...
            while true do
                ...
                skip()
                if text:sub(pos, pos) ~= ":" then error("expected ':' after JSON object key") end
                pos = pos + 1
                obj[key] = parseValue()
                skip()
        ...
        if not number or #number == 0 then error("invalid JSON value near position " .. tostring(pos)) end
        pos = pos + #number
    ...
    skip()
    if pos <= len then error("trailing JSON data near position " .. tostring(pos)) end
    return value
end
...
local function getResolve()
    -- Try the Fusion scripting API first, then the Resolve app API.
    if bmd and bmd.scriptapp then
        local ok, result = pcall(function() return bmd.scriptapp("Resolve") end)
...
local function importMissing(mediaPool, root, names)
    ...
    for _, name in ipairs(names) do
        ...
        if item then lookup[name] = item else paths[#paths + 1] = CONFIG.mediaDir .. "/" .. name end
    end
    if #paths > 0 then
        log("IMPORTING " .. tostring(#paths) .. " MEDIA ITEMS")
        local imported = mediaPool:ImportMedia(paths)
        if not imported then error("Resolve could not import required media.") end
        for _, item in ipairs(imported) do lookup[item:GetName()] = item end
    end
    for _, name in ipairs(names) do
        if not lookup[name] then lookup[name] = findRootItem(root, name) end
        if not lookup[name] then error("Media Pool item not found: " .. name) end
    end
    return lookup
end
...
local function append(mediaPool, mediaItem, inFrame, outFrame, recordFrame, trackIndex, mediaType, label)
    ...
    if not ok then error(label .. " failed: " .. tostring(result)) end
    if not result or #result == 0 then error(label .. " failed: Resolve returned no timeline item") end
    return result[1]
end

local function main()
    log("ACQUIRING RESOLVE API")
    local resolve = getResolve()
    if not resolve then error("Could not obtain Resolve API") end
    log("RESOLVE API OK")

    local projectManager = resolve:GetProjectManager()
    if not projectManager then error("Project Manager unavailable") end
    local project = projectManager:GetCurrentProject()
    if not project then error("Open a Resolve project first") end
    log("PROJECT = " .. tostring(project:GetName()))

    log("READING ASSEMBLY")
    local assembly = JSON.decode(readFile(CONFIG.assembly))
    if type(assembly) ~= "table" then error("Invalid resolve_assembly.json") end
    log("ASSEMBLY OK")

    local mediaPool = project:GetMediaPool()
    if not mediaPool then error("Media Pool unavailable") end
    ...
    local names = collectNames(assembly)
    log("REQUIRED MEDIA = " .. tostring(#names))
    ...
    -- Prefer a zero-based timeline start when supported. Some Resolve builds
    -- may not expose SetStartTimecode; in that case continue rather than fail.
    local startTcSupported = false
    local startTcResult = pcall(function()
        if timeline.SetStartTimecode then
            local result = timeline:SetStartTimecode("00:00:00:00")
            startTcSupported = (result == true)
        end
    end)
    log("SET START TIMECODE supported=" .. tostring(startTcSupported) .. " callOk=" .. tostring(startTcResult))

    project:SetCurrentTimeline(timeline)
    ensureTracks(timeline)

    local recordFrame = 0
    local mainPlaced, audioPlaced, brollPlaced, musicPlaced = 0, 0, 0, 0
    ...
    for idx, item in ipairs(assembly.mainTimeline or {}) do
        ...
        if duration <= 0 then error("Invalid source range in mainTimeline[" .. idx .. "]") end
    ...
    for idx, item in ipairs(assembly.brollOverlays or {}) do
        ...
        if sourceOut >= sourceIn then
            append(mediaPool, mediaItem, sourceIn, sourceOut, record, 2, 1, "V2 B-roll[" .. idx .. "]")
        ...
        for idx, cue in ipairs(assembly.musicCues or {}) do
            ...
            if duration > 0 then
                local ok, result = pcall(function()
                    return append(mediaPool, musicItem, 0, duration - 1, record, 2, 2, "A2 Music[" .. idx .. "]")
                end)
                if ok and result then musicPlaced = musicPlaced + 1 else log("WARNING: skipping A2 Music[" .. idx .. "]: " .. tostring(result)) end
            end
        end
    end
...
Original media modified: NO]],
        timelineName,
        mainPlaced, #(assembly.mainTimeline or {}),
        audioPlaced, #(assembly.mainTimeline or {}),
        brollPlaced, #(assembly.brollOverlays or {}),
        musicPlaced, #(assembly.musicCues or {}),
